...
class IcCalculator:
    ...
    def calculate_ic_from_samples(self, samples: typing.Sequence[Sample]) -> typing.Mapping[hpotk.TermId, float]:
        ...
        self._sample_array = self._get_sample_array()
    ...
    def _get_sample_array(self) -> np.array:
        ...
        array_type = [(col.value, bool) for col in self._sample_terms]
        array = np.zeros(len(self._samples), dtype=array_type)
        i = 0
    ...
    def _get_term_ic(self, term: hpotk.TermId) -> (hpotk.TermId, float):
        ...
        relevant_descendants = [i.value for i in list(term_descendants & self._sample_terms)]
        if len(relevant_descendants) > 0:
            freq = sum(1 if any(row[relevant_descendants]) else 0 for row in self._sample_array)
        else:
            freq = 1
        ic = log(len(self._sample_array) / freq)
        return term, ic
